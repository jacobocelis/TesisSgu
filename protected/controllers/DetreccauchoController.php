...
class DetreccauchoController extends Controller
{
	/**
                ...
                if (Yii::app()->request->isAjaxRequest){
					if(isset($_POST['idfac'])){
						...
						for($i=0;$i<count($actividades);$i++){
							$recursos=Detreccaucho::model()->findAll(array("condition"=>"iddetalleEventoCa = '".$actividades[$i]["iddetalleEventoCa"]."'"));
	...
	{
		$dataProvider=new CActiveDataProvider('Detreccaucho');
	...
	{
		$model=new Detreccaucho('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Detreccaucho']))
			$model->attributes=$_GET['Detreccaucho'];
	 ...
	 * @return Detreccaucho the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Detreccaucho::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Detreccaucho $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='detreccaucho-form')
		{
