	 ...
	 */
		...
		if(isset($_POST['Modelo'])){
			...
			if (Yii::app()->request->isAjaxRequest){
				echo CJSON::encode(array(
					'status'=>'failure', 
					'div'=>$this->renderPartial('_form', array('model'=>$model,'idmarca'=>$id), true)));
				exit;               
			}
		 
	}
	public function actionActualizarListaModelo($id){
	
			$lista=Modelo::model()->findAll('idmarca='.$id.' order by id desc');
			foreach($lista as $li){
