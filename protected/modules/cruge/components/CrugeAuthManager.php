...
public function getUsersAssigned2($itemName)
    {
        $rows = $this->db->createCommand('select re.id from sgu_caracteristicavehgrupo cg, 
		sgu_grupo gu, sgu_repuesto re where cg.idgrupo=gu.id and gu.grupo="'.$itemName.'" 
		and re.id=cg.idrepuesto;')->queryAll();          
        $item = array();
        if ($rows != null) {
            foreach ($rows as $row) {
                if (!in_array($row['id'], $item)) {
                    $item[] = $row['id'];
                }
            }
        }
